...
public class HelloServlet extends HttpServlet {
	...
	/*
	 	4. doGet()
	 		...
	 		2) GET 방식의 요청의 예시 1,2 html태그 3,4,5 자바스크립트 
	 			(!) <a href="http://localhost:9090/01_Servlet/HelloServlet">
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 응답 : 서버에서 클라이언트로 보내주는건 리스판스 , 클라이언트가 서버쪽으로 보내주는건 리퀘스트
		// getWriter :  텍스트를 보내주는것 출력 스트림 (글자만 보내는 통로) html태그 
		// 응답 정보를 가진 객체 : response (선언이 되있어야 사용가능)
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// POST 메소드를 통해서 넘어온 정보를 모두 doGet() 메소드에 넘긴다. (나 안하려고 놀려고) 
