...
public class UploadServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		MultipartRequest muitpartRequest = new MultipartRequest(
		...
		// 요청 파라미터 (MultipartRequest muitpartRequest를 이용해서 파라미터를 가져와야 한다.)
		String uploader = muitpartRequest.getParameter("uploader");
		String originName = muitpartRequest.getOriginalFileName("filename");   // 원래 첨부 파일명
		String filesystemName = muitpartRequest.getFilesystemName("filename"); // 저장된 첨부 파일명
		
		// 첨부 파일 정보
		// 파일명, 경로, 최종수정일(yyyy-MM-dd), 파일크기(KB)
		File file = muitpartRequest.getFile("filename");
