...
public class BoardDAO {
	...
	try {
			...
			// 6. PreparedStatement 객체를 이용해 쿼리문 실행 (INSERT문 실행은 executeQuery 메소드로 한다.)
	...
	public int updateBoard(BoardDTO board) {
		...
		try {
			...
			// 3. 실행할 쿼리문 
			sql = "UPDATE BOARD SET TITLE = ?, CONTENT = ?, MODIFIED_DATE = SYSDATE WHERE BOARD_NO = ?"; // 제목 내용 변수값을 바꿔주고 수정일은 현재날짜로 
			...
			// 6. PreparedStatement 객체를 이용해 쿼리문 실행 (DELETE문 실행은 executeQuery 메소드로 한다.)
	...
	public int delecteBoard(int board_no) {
		...
		try {
			...
			// 3. 실행할 쿼리문 
			sql = "DELETE FROM BOARD WHERE BOARD_NO = ?"; // 번호가 일치하면 삭제하기
