...
public class PostDAO { 
	...
	public PostVO getPostByNo(int post_no) throws Exception { // 상세보기
		PostVO post = null;
		con = dataSource.getConnection();
		sql = "SELECT POST_NO, WRITER, TITLE, CONTENT, IP, MODIFIED_DATE, CREATED_DATE";
		sql += "  FROM POST";
		...
		// 자원의 반납
		close();
		return post; // 방금 만든 메소드 반환 
	}
	
	public int updatePost(PostVO post) throws Exception { // 수정
		con = dataSource.getConnection();
		sql = "UPDATE POST";
	...
	public int deletePost(int post_no) throws Exception {  // 삭제
		con = dataSource.getConnection();
		sql = "DELETE FROM POST WHERE POST_NO = ?";
		ps = con.prepareStatement(sql);
		ps.setInt(1, post_no); // ? 채우기 
		int deleteResult = ps.executeUpdate(); // 실행
		close();
		return deleteResult;
	}
}